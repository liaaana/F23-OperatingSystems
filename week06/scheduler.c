...
void create_process(int new_process) {
    // stopping the running process omitted because it is non-preemptive FCFS algorithm
...
ProcessData find_next_process() {
    ...
    // Finding location of the next process to run from the {data} array
...
void schedule_handler(int signum) {
    ...
    if (running_process == -1) {
        ...
        create_process(running_process); // 3.C.1. then create a new process for {running_process} and print the message
        printf("Scheduler: Starting Process %d (Remaining Time: %d)\n", running_process, data[running_process].burst);
    }

    // 3. 3.A. 3.C.2 were omitted because it is non-preemptive FCFS algorithm
}
